...
public class MapstructdemoApplication implements CommandLineRunner{
	...
	public void run(String... strings) throws Exception {
		final Logger logger = LoggerFactory.getLogger(MapstructdemoApplication.class);
...
//	        ProductDetailRepository repository = ctx.getBean(ProductDetailRepository.class);
		...
		List<ProductDetail> productDetailList = repository.findByCompany(company);


		CompanyProduct cp = CompanyProductMapper.Instance.CompanyEntityToCompanyProduct(company, productDetailList);

		for(ProductInfo pi: cp.getProducts()) {
			logger.info(cp.getCompanyId());
			logger.info(pi.getProductId());
		}
